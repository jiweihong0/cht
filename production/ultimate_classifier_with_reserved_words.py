...
class UltimateClassifier:
    ...
    def load_configs(self):
        ...
        try:
            # 載入保留詞配置
            if os.path.exists(self.reserved_words_config):
                with open(self.reserved_words_config, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                    self.reserved_words = config.get('reserved_words', {})
                print(f"✅ 載入保留詞配置：{len(self.reserved_words)} 個詞彙")
            else:
                print(f"⚠️ 保留詞配置文件不存在：{self.reserved_words_config}")
                self.reserved_words = {}
            
            # 載入分類規則配置
            if os.path.exists(self.category_rules_config):
                with open(self.category_rules_config, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                    self.category_rules = config.get('category_rules', {})
                print(f"✅ 載入分類規則配置：{len(self.category_rules)} 個類別")
            else:
                print(f"⚠️ 分類規則配置文件不存在：{self.category_rules_config}")
                self.category_rules = {}
    ...
    def save_configs(self):
        ...
        try:
            # 保存保留詞配置
            reserved_config = {'reserved_words': self.reserved_words}
            with open(self.reserved_words_config, 'w', encoding='utf-8') as f:
                json.dump(reserved_config, f, ensure_ascii=False, indent=2)
            
            # 保存分類規則配置
            rules_config = {'category_rules': self.category_rules}
